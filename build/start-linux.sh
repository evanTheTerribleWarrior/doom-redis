#!/bin/bash
# start-linux.sh - Build script for Debian/Ubuntu Linux

set -e

if [ "$(uname)" != "Linux" ] || [ ! -f /etc/debian_version ]; then
  echo "[Error] This script is intended for Debian/Ubuntu Linux only."
  exit 1
fi
...
REQUIRED_PKGS=(cmake build-essential libsdl2-dev libsdl2-mixer-dev libhiredis-dev python3 python3-pip python3-venv git lsof)
MISSING_PKGS=()
...
if ! command -v npm >/dev/null 2>&1; then
  echo "[Deps] Installing Node.js and npm..."
