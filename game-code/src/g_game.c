...
{ 
    ...
    { 
        // Redis chat mode
        // 1. Upon Enter we remove the chat line and clear it
        // 2. Upon Backspace we remove characters as needed
        // 3. If 'c' is pressed we initialise everything
        if (redischatmode) {
            if (ev->type == ev_keydown) {
                if (ev->data1 == KEY_ENTER || ev->data1 == '\r') {
                    if (redischatlen > 0) {
                        AddChatEvent(mainContext, players[consoleplayer].playerName, redischatbuffer);
                    }
                    redischatmode = false;
                    redischatlen = 0;
                    redischatbuffer[0] = '\0';
                ...
                else if (ev->data1 == KEY_BACKSPACE || ev->data1 == 127) {
                    if (redischatlen > 0) {
                        HUlib_delCharFromTextLine(&w_redischatinput);
                        redischatlen--;
                        redischatbuffer[redischatlen] = '\0';
                        
                    }
                    
                    return true;
                }
                else if (redischatlen < 127 && ev->data1 >= 32 && ev->data1 <= 126) {
                    redischatbuffer[redischatlen++] = ev->data1;
                    HUlib_addCharToTextLine(&w_redischatinput, ev->data1);
                    redischatbuffer[redischatlen] = '\0';
                    return true;
                }
            }
        ...
        // Activate chat mode on key (e.g., 'c')
        if (ev->type == ev_keydown && ev->data1 == 'c') {
            redischatmode = true;
            redischatlen = 0;
            redischatbuffer[0] = '\0';
            HUlib_clearTextLine(&w_redischatinput);
...
{ 
    ...
    int		itemcount;
    int		secretcount; 
    char savedName[MAX_PLAYER_NAME];
	 
    memcpy (frags,players[player].frags,sizeof(frags)); 
    killcount = players[player].killcount; 
    itemcount = players[player].itemcount; 
    secretcount = players[player].secretcount; 
	strncpy(savedName, players[player].playerName, MAX_PLAYER_NAME);
    ...
    strncpy(players[player].playerName, savedName, MAX_PLAYER_NAME);
