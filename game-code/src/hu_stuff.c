...
void DrawRedisNotification() {
    if(redisNotificationMode)
    {
        HUlib_clearTextLine(&w_redisNotification);
...
void HU_Drawer(void)
{
    ...
    // Draw Line for Redis Chat
    if (redischatmode)
    HUlib_drawTextLine(&w_redischatinput, false);

    if(redisNotificationMode)
    HUlib_drawTextLine(&w_redisNotification, false);
...
void HU_Erase(void)
{
    ...
    //Redis
    HUlib_eraseTextLine(&w_redischatinput);
    HUlib_eraseTextLine(&w_redisNotification);
...
void HU_Ticker(void)
{
    ...
    // Redis tick down notification counter
    if (redisNotificationCounter && !--redisNotificationCounter)
    {
        redisNotificationMode = false;
        HUlib_clearTextLine(&w_redisNotification);
    }
